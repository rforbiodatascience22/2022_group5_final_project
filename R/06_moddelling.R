...
gene_expression_data %>% 
  lm(outcome ~ ENSG00000227232.5, 
      data=.) %>% 
  summary()


gene_expression_data %>% 
  multinom(outcome ~ ENSG00000227232.5, 
     data=.) %>% 
  summary()

multinom(outcome ~ Alcohol + Color -1, data = sample_attributes_tissue)
...
expression_data_model_tidy <- expression_data_grouped %>%
  mutate(mdl = map(data, ~lm(outcome ~ readcount,
                              data = .x,
                              ))) %>% 
